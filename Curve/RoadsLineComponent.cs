...
namespace Moth
{
    public class RoadsLineComponent : GH_Component
    {
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            //Join the Curves
            Curve[] cr = Curve.JoinCurves(crvs);

            //Offset Curves Sharp
            if (n == 0)
            {
                //Create Regions for Every Pair of Curves
                for (int j = 0; j < cr.Length; j += 2)
                {
                    //Create Empty List of the curves of the region
                    List<Curve> cc = new List<Curve>();
                    //Add to the list the pair of curves
                    cc.Add(cr[j]);
                    cc.Add(cr[j + 1]);
                    //Create new lines and add them to the list
                    LineCurve ln1 = new LineCurve(cr[j].PointAtStart, crvs[j + 1].PointAtStart);
                    LineCurve ln2 = new LineCurve(cr[j].PointAtEnd, crvs[j + 1].PointAtEnd);
                    cc.Add(ln1);
                    cc.Add(ln2);
                    //Join Curves and Create a Region
                    regions.AddRange(Curve.JoinCurves(cc));
                }
            }
            ...
            else if (n == 2)
            {
                for (int j = 0; j < cr.Length; j += 2)
                {
                    ...
                    if (cr[j].IsPolyline() == false)
                    {
                        ...
                        Curve c1 = ReparametrizeCurve(cr[j]);
                        Curve c2 = ReparametrizeCurve(cr[j + 1]);
            ...
            List<Curve> nurbs = new List<Curve>();
            ...
            // Join curves
            List<Curve> joinedCurves = new List<Curve>(Curve.JoinCurves(nurbs, 0.008));

            //Boolean Region of the closed Curves
            //Perform boolean union on the joined curves
            List<Curve> unionResult = Curve.CreateBooleanUnion(joinedCurves, 0.001).ToList();

            //Output
            DA.SetDataList(0, unionResult);
        }
